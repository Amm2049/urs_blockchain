...
export default function Navbar() {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      console.error('Navbar connect check error', err);
    }
  };
          ...
          <nav className="hidden md:flex items-center gap-1">
            {items.map(({ to, label, Icon }) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-150 ${isActive
        ...
        <div className="md:hidden flex overflow-x-auto gap-1 px-4 pb-2 no-scrollbar">
          {items.map(({ to, label, Icon }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex-shrink-0 flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium transition-all ${isActive ? 'bg-brand-500/20 text-brand-400' : 'text-white/50 hover:text-white'
                }`
              }
            >
